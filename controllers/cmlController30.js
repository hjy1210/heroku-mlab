...
var getScore = require('./getscore30')
    ...
    .exec(function (err, list){
      ...
      for (var i=0;i<list.length;i++) {
        ...
        for (var y in corrects[x]) {
          if (corrects[x][y]['qti-mapping']) {
            corrects[x][y]['qti-mapping'] = corrects[x][y]['qti-mapping'].replace(/>\s*?</g, "><")
            corrects[x][y]['qti-mapping'] = new DOMParser().parseFromString(corrects[x][y]['qti-mapping']).documentElement
          }
        }
