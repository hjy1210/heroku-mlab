...
function generatePsetInfo(psets) {
  var info = []
  ...
  exports.testform_get = function (req, res, next) {
    Pset.find({})
      .exec(function (err, list_psets) {
        ...
        var psetinfo = generatePsetInfo(psets)
        res.render('testform', { psets: psets, psetinfo: psetinfo });
      });
  };

  exports.testform_post = function (req, res, next) {
    //console.log(req.body)
    var data = req.body.psetinfo
    var userpsets = JSON.parse(data)  // [{code,stdans:[{len,type,score,userans}]}]
    Pset.find({})
      .exec(function (err, list_psets) {
        if (err) { return next(err); }
        //Successful, so render
        ...
        var codes = psets.map(x => x.code)
        for (var i = 0; i < userpsets.length; i++) {
          var code = userpsets[i].code
          var psetindex = codes.indexOf(code)
          for (var j = 0; j < userpsets[i].stdans.length; j++) {
            var type = userpsets[i].stdans[j].type
            var count = psets[psetindex].items && psets[psetindex].items[j] && psets[psetindex].items[j].choices ? psets[psetindex].items[j].choices.length : 0
            var score = userpsets[i].stdans[j].score
            var std=psets[psetindex].stdans[j].ans
            var user=userpsets[i].stdans[j].userans
            var scoreGot = getScore(type, count, score, std, user)
            userpsets[i].stdans[j].scoreGot = scoreGot
            userpsets[i].stdans[j].ans=psets[psetindex].stdans[j].ans
          }
        }
        res.render('report', {userpsets:userpsets})
        //res.send(JSON.stringify(userpsets, null, 2))
      })
  }
